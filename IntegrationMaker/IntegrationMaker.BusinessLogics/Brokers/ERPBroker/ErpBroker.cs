...
namespace IntegrationMaker.BusinessLogic.Brokers.ERPBroker
{
    public abstract class ErpBroker
    {
        ...
        public IntegrationCredentialModel Credential { get; set; }
        ...
        public static ErpBroker GetInstance(IntegrationModel integration, IntegrationCredentialModel credential, Job job = null)
        {
            var integrationType = IntegrationTypes.Single(x => x.Id == integration.IntegrationTypeId);
            Type type = null;
            var assembly = Assembly.GetExecutingAssembly();
            if(integrationType.Name == "economic")
            type = assembly.GetTypes().First(t => t.Name == "EconomicBroker");
            if(integrationType.Name == "Dynamic 365 Bussines Central")
                type = assembly.GetTypes().First(t => t.Name == "Dynamic365Broker");
